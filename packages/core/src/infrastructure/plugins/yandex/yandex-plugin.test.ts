...
import { registerYandexPlugin, yandexManifest } from './yandex-plugin.js';
...
async function bench(options: BenchOptions = {}) {
  ...
  await registerYandexPlugin(registry, runtime, { retryDelaysMs: [50, 100], secure: false });
...
describe('the Yandex Station plugin', () => {
  it('stays off until somebody turns it on', async () => {
    ...
    assert.equal(bed.runtime.status('yandex')?.status, 'off');
    await bed.dispose();
  });
  ...
  it('says what is missing rather than just failing', async () => {
    ...
    await bed.until('the plugin to ask for a sign-in', () => {
      const status = bed.runtime.status('yandex');
  ...
  it('connects to a remembered speaker and publishes what it plays', async () => {
    ...
    await bed.until('the track to appear', () => bed.variables.get('yandex.title') === 'TAKE ME');

    assert.equal(bed.variables.get('yandex.artist'), 'D A N N Y');
    assert.equal(bed.variables.get('yandex.playing'), true);
    // Reported as a person reads it, not as the speaker sends it.
    assert.equal(bed.variables.get('yandex.volume'), 40);
    assert.equal(bed.variables.get('yandex.speaker'), 'Яндекс станция');
    assert.equal(bed.variables.get('yandex.source'), 'Playlist');
    ...
    assert.equal(bed.variables.get('yandex.title(office)'), 'TAKE ME');
    assert.equal(bed.variables.get('yandex.playing(office)'), true);
    assert.equal(bed.variables.get('yandex.connected(office)'), true);
    assert.equal(bed.runtime.status('yandex')?.status, 'ready');
  ...
  it('writes the position as a clock as well as a number', async () => {
    const bed = await bench({ state: playingState() });
    await bed.until('the position', () => bed.variables.get('yandex.progress') === 16);

    assert.equal(bed.variables.get('yandex.progress-time'), '0:16');
    assert.equal(bed.variables.get('yandex.progress-time(office)'), '0:16');
    // The pair a label is written from: "0:16 / 2:09".
    assert.equal(bed.variables.get('yandex.duration-time'), '2:09');
    ...
    await bed.until('the longer position', () => bed.variables.get('yandex.progress') === 3727);
    assert.equal(bed.variables.get('yandex.progress-time'), '1:02:07');
  ...
  it('offers our speakers and nobody else’s', async () => {
    ...
    await bed.until('the connection', () => bed.runtime.status('yandex')?.status === 'ready');

    const offered = await bed.runtime.optionsFor('yandex', 'speakers');
  ...
  it('never touches a speaker that is not in the account', async () => {
    ...
    await bed.until('the connection to settle', () => bed.runtime.status('yandex')?.status === 'ready');
    // The stranger's port would refuse anything; reaching it at all is the bug.
    await failsWith(() => bed.run('yandex.next-track', { speaker: 'stranger' }), /not connected/i);
  ...
  it('pauses what is playing and starts what is not', async () => {
    ...
    await bed.until('the state to arrive', () => bed.variables.get('yandex.playing') === true);

    await bed.run('yandex.play-pause', { mode: 'toggle' });
    ...
    assert.equal(bed.variables.get('yandex.playing'), false);
  ...
  it('moves the volume by a step the speaker can actually keep', async () => {
    const bed = await bench({ state: playingState() });
    await bed.until('the volume to arrive', () => bed.variables.get('yandex.volume') === 40);

    await bed.run('yandex.volume', { mode: 'up', value: 15 });
  ...
  it('seeks from where the track actually is', async () => {
    const bed = await bench({ state: playingState() });
    await bed.until('the position to arrive', () => bed.variables.get('yandex.progress') === 16);

    await bed.run('yandex.rewind', { mode: 'forward', seconds: 30 });
    await bed.until('a command to reach the speaker', () => bed.sent().length > 0);
  ...
  it('cycles repeat through the modes the speaker names', async () => {
    const bed = await bench({ state: playingState() });
    await bed.until('the state to arrive', () => bed.variables.get('yandex.source') === 'Playlist');

    await bed.run('yandex.repeat', { mode: 'cycle' });
    await bed.until('a command to reach the speaker', () => bed.sent().length > 0);
  ...
  it('keeps a spoken phrase apart from a command to Alice', async () => {
    ...
    await bed.run('yandex.command', { text: 'включи ночник' });
    ...
    await bed.run('yandex.say', { text: 'привет' });
  ...
  it('draws the album art of whatever is on', async () => {
    ...
    try {
      await bed.until('the track', () => bed.variables.get('yandex.title') === 'TAKE ME');
  ...
  it('refuses to guess which speaker a key meant', async () => {
    ...
    await failsWith(() => bed.run('yandex.next-track', { speaker: 'nobody' }), /No speaker/i);
