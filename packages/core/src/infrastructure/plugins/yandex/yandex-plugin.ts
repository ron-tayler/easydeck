...
export const YANDEX_PLUGIN_ID = 'yandex';
...
const FAMILIES = [
  ...
  'yandex.volume',
  'yandex.progress',
  'yandex.progress-time',
  'yandex.duration',
  'yandex.duration-time',
  'yandex.source',
] as const;
...
export const yandexManifest: PluginManifest = {
  ...
  variables: [
    ...
    {
      name: 'yandex.progress',
      type: 'number',
      label: { en: 'Position, s', ru: 'Позиция, с' },
      argument: speakerArgument(),
    },
    {
      name: 'yandex.duration',
      type: 'number',
  ...
  actions: [
    ...
    {
      type: 'yandex.next-track',
      icon: 'next',
      label: { en: 'Next track', ru: 'Следующий трек' },
      params: [speakerParam()],
    },
    {
      type: 'yandex.prev-track',
      icon: 'previous',
        ...
        states: [
          {
            ...
            visual: {
              background: '#1a1c22',
              surface: { type: 'yandex.cover' },
              label: { text: '{{yandex.title}}', fontSize: 11 },
            },
            actions: { press: [{ type: 'yandex.play-pause', params: { mode: 'toggle' } }] },
          },
          {
            id: 'playing',
            when: true,
        ...
        states: [
          {
            id: 'paused',
            when: false,
            visual: { background: '#22303c', icon: { source: 'plugin:yandex/play.svg' } },
            actions: { press: [{ type: 'yandex.play-pause', params: { mode: 'toggle' } }] },
          },
          {
            id: 'playing',
            when: true,
            visual: { background: '#2f6f4f', icon: { source: 'plugin:yandex/pause.svg' } },
            actions: { press: [{ type: 'yandex.play-pause', params: { mode: 'toggle' } }] },
          },
        ],
      },
...
export class YandexPlugin implements Plugin {
  ...
  start(host: PluginHost): void {
    ...
    host.provideSurface('yandex.cover', async (request) => this.cover(request));
  ...
  private onConnection(
    ...
    host.setFamily('yandex.connected', speaker.deviceId, state === 'ready');
    if (speaker.deviceId === this.chosen()?.deviceId) {
      host.setFamily('yandex.connected', NO_SPEAKER, state === 'ready');
    }
  }
  ...
  private publish(deviceId: string, state: SpeakerState): void {
    ...
    const values: Record<string, VariableValue | undefined> = {
      'yandex.playing': state.playing === true,
      'yandex.volume': state.volume === undefined ? undefined : Math.round(state.volume * 100),
      'yandex.title': player?.title ?? '',
      'yandex.artist': player?.subtitle ?? '',
      'yandex.progress': round(player?.progress),
      'yandex.progress-time': asClock(player?.progress),
      'yandex.duration': round(player?.duration),
      'yandex.duration-time': asClock(player?.duration),
      'yandex.source': player?.entityInfo?.type ?? '',
    };
    ...
    if (deviceId === this.chosen()?.deviceId) {
      host.setVariable('yandex.speaker', this.chosen()?.name ?? '');
    }
  }
  ...
  private clearVariables(): void {
    ...
    host.setVariable('yandex.speaker', undefined);
  ...
  handlers(): Record<string, ActionHandler> {
    return {
      'yandex.play-pause': async (params) => {
        ...
        this.expect(deviceId, 'yandex.playing', playing);
      },

      'yandex.next-track': async (params) => {
        this.send(this.target(params).deviceId, { command: 'next' });
      },

      'yandex.prev-track': async (params) => {
        this.send(this.target(params).deviceId, { command: 'prev' });
      },

      'yandex.rewind': async (params) => {
        const { deviceId, state } = this.target(params);
      ...
      'yandex.volume': async (params) => {
        ...
        this.expect(deviceId, 'yandex.volume', Math.round(volume * 100));
      },

      'yandex.repeat': async (params) => {
        const { deviceId, state } = this.target(params);
...
export async function registerYandexPlugin(
  registry: ActionRegistry,
  runtime: PluginRuntime,
  options: YandexPluginOptions = {},
): Promise<void> {
  const plugin = new YandexPlugin(options);

  registry.installPlugin(yandexManifest, plugin.handlers());
  await runtime.install(yandexManifest, plugin);
  runtime.registerCommands(YANDEX_PLUGIN_ID, {
    login: () => plugin.login(),
