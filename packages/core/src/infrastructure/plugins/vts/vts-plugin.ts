import { PLUGIN_API_VERSION, parseVariableKey, stringParam } from '@easydeck/engine';
import type {
  ActionHandler,
  ActionRegistry,
  ParamOption,
...
export const VTS_PLUGIN_ID = 'vts';
  ...
  presets: [
    {
      ...
      button: {
         ...
         */
        stateFrom: 'vts.expression',
        states: [
          {
            id: 'off',
            when: false,
            visual: { background: '#22303c', label: { text: 'Выражение', fontSize: 13 } },
            actions: { press: [{ type: 'vts.set-expression', params: { mode: 'toggle' } }] },
          },
          {
            id: 'on',
            when: true,
...
export class VtsPlugin implements Plugin {
  ...
  start(host: PluginHost): void {
    ...
    host.onWatched((keys) => {
      this.watching = keys
        ...
        .filter((parsed) => parsed.family === 'vts.expression' && parsed.argument)
  ...
  private connect(): void {
    ...
    this.connection = new VtsConnection({
      ...
      onState: (state, message) => {
        ...
        host.setVariable('vts.connected', state === 'ready');
  ...
  private async readEverything(): Promise<void> {
    ...
    try {
      ...
      host.setVariable('vts.model', model['modelLoaded'] === true ? String(model['modelName'] ?? '') : '');
  ...
  private async readExpressions(): Promise<void> {
    ...
    try {
      ...
      for (const file of this.watching) {
        ...
        host.setFamily('vts.expression', file, byFile.get(file)?.active);
      }
    } catch (cause) {
  ...
  handlers(): Record<string, ActionHandler> {
    return {
      ...
      'vts.set-expression': async (params) => {
        ...
        if (this.watching.includes(file)) this.host?.setFamily('vts.expression', file, active);
      },

      'vts.load-model': async (params) => {
...
export async function registerVtsPlugin(
  registry: ActionRegistry,
  runtime: PluginRuntime,
  options: VtsPluginOptions = {},
): Promise<void> {
  const plugin = new VtsPlugin(options);

  registry.installPlugin(vtsManifest, plugin.handlers());
  await runtime.install(vtsManifest, plugin);
  runtime.registerCommands(VTS_PLUGIN_ID, {
