...
import { registerVtsPlugin, vtsManifest } from './vts-plugin.js';
...
async function bench(options: { token?: string; stored?: string; enabled?: boolean } = {}) {
  ...
  await registerVtsPlugin(registry, runtime, { retryDelaysMs: [50, 100] });
  ...
  return {
    ...
    async waitUnauthorised() {
      ...
      while (Date.now() < deadline) {
        const state = runtime.status('vts');
...
describe('the VTube Studio plugin', () => {
  it('stays off until somebody turns it on', async () => {
    ...
    assert.equal(bed.runtime.status('vts')?.status, 'off');
  ...
  it('waits to be authorised rather than throwing a dialog at whoever is live', async () => {
    ...
    await bed.until('the plugin to report it needs authorising', () => {
      const state = bed.runtime.status('vts');
  ...
  it('authorises when asked, keeps the token, and connects with it', async () => {
    ...
    await bed.runtime.runCommand('vts', 'authorise');
    ...
    await bed.until('the model to be read', () => bed.variables.get('vts.model') === 'Аканэ');
    assert.equal(bed.variables.get('vts.connected'), true);

    // Kept where every other secret is kept, so the next start is silent.
    assert.deepEqual(await bed.settings.filledSecrets('vts'), ['token']);
    assert.equal((await bed.settings.load('vts'))['token'], TOKEN);
  ...
  it('says so when the user presses Deny', async () => {
    ...
    await assert.rejects(bed.runtime.runCommand('vts', 'authorise'), /denied|refused/i);
    assert.equal(bed.runtime.status('vts')?.status, 'error');
...
describe('what the VTube Studio plugin offers and does', () => {
  it('offers the hotkeys, expressions and models it can see', async () => {
    ...
    const hotkeys = await bed.runtime.optionsFor('vts', 'hotkeys', {});
    const expressions = await bed.runtime.optionsFor('vts', 'expressions', {});
    const models = await bed.runtime.optionsFor('vts', 'models', {});
  ...
  it('answers with an empty list when VTube Studio is closed, so a key can still be set up', async () => {
    const bed = await bench({ enabled: false });
    assert.deepEqual(await bed.runtime.optionsFor('vts', 'hotkeys', {}), []);
    await bed.dispose();
  });
  ...
  it('triggers a hotkey and loads a model', async () => {
    ...
    await bed.registry.run(
      { type: 'vts.trigger-hotkey', params: { hotkey: 'hk-1' } },
      context(bed.variables),
    );
    await bed.registry.run(
      { type: 'vts.load-model', params: { model: 'model-2' } },
      context(bed.variables),
    );
  ...
  it('refuses an action while VTube Studio is not there, rather than doing nothing', async () => {
    ...
    await assert.rejects(
      bed.registry.run({ type: 'vts.trigger-hotkey', params: { hotkey: 'hk-1' } }, context(bed.variables)),
      (error: Error) => {
  ...
  it('toggles an expression against what it is now, not against what a key thinks', async () => {
    ...
    await bed.registry.run(
      { type: 'vts.set-expression', params: { expression: 'angry.exp3.json', mode: 'toggle' } },
      context(bed.variables),
    );
  ...
  it('reads only the expressions a profile actually uses', async () => {
    ...
    bed.runtime.setWatched(['vts.expression(angry.exp3.json)']);
    await bed.until('the watched expression', () => bed.variables.get('vts.expression(angry.exp3.json)') === true);

    assert.equal(bed.variables.get('vts.expression(blush.exp3.json)'), undefined);
  ...
  it('follows the model changing under it, whoever caused it', async () => {
    ...
    await bed.until('the new model', () => bed.variables.get('vts.model') === 'Котик');

    bed.vts.emit('TrackingStatusChangedEvent', { faceFound: true });
    await bed.until('tracking', () => bed.variables.get('vts.tracking') === true);
  ...
  it('clears what it published when the connection goes', async () => {
    ...
    await bed.vts.close();
    await bed.until('the model to be cleared', () => bed.variables.get('vts.model') === undefined);
    assert.equal(bed.variables.get('vts.connected'), false);
