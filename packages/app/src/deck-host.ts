...
export class DeckHost extends EventEmitter<DeckHostEvents> implements ApiSource {
  ...
  pluginSettings(pluginId: string) {
    return this.require().pluginSettings(pluginId);
  }

  savePluginSettings(pluginId: string, values: Readonly<Record<string, VariableValue>>) {
    return this.require().savePluginSettings(pluginId, values);
  }
  ...
  pluginStatuses() {
    ...
    return this.deck?.pluginStatuses() ?? {};
  }
  ...
  private forward(deck: DeckService): void {
    ...
    deck.on('pluginStatusChanged', (event) => this.emit('pluginStatusChanged', event));
  }
